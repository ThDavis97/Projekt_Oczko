...
\begin{DoxyPre}
 ...
 \doxyref{event\+\_\+groups.\+h}{p.}{event__groups_8h} 
\begin{DoxyPre}
   EventBits\_t \doxyref{xEventGroupWaitBits(     EventGroupHandle\_t xEventGroup,                                     const EventBits\_t uxBitsToWaitFor,                                     const BaseType\_t xClearOnExit,                                     const BaseType\_t xWaitForAllBits,                                     const TickType\_t xTicksToWait )}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db};
\end{DoxyPre}
...
\hline
{\em x\+Clear\+On\+Exit} & If x\+Clear\+On\+Exit is set to pd\+TRUE then any bits within ux\+Bits\+To\+Wait\+For that are set within the event group will be cleared before \doxyref{x\+Event\+Group\+Wait\+Bits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returns if the wait condition was met (if the function returns for a reason other than a timeout). If x\+Clear\+On\+Exit is set to pd\+FALSE then the bits set in the event group are not altered when the call to \doxyref{x\+Event\+Group\+Wait\+Bits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returns.\\
\hline
{\em x\+Wait\+For\+All\+Bits} & If x\+Wait\+For\+All\+Bits is set to pd\+TRUE then \doxyref{x\+Event\+Group\+Wait\+Bits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} will return when either all the bits in ux\+Bits\+To\+Wait\+For are set or the specified block time expires. If x\+Wait\+For\+All\+Bits is set to pd\+FALSE then \doxyref{x\+Event\+Group\+Wait\+Bits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} will return when any one of the bits set in ux\+Bits\+To\+Wait\+For is set or the specified block time expires. The block time is specified by the x\+Ticks\+To\+Wait parameter.\\
\hline
...
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The value of the event group at the time either the bits being waited for became set, or the block time expired. Test the return value to know which bits were set. If \doxyref{x\+Event\+Group\+Wait\+Bits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returned because its timeout expired then not all the bits being waited for will be set. If \doxyref{x\+Event\+Group\+Wait\+Bits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returned because the bits it was waiting for were set then the returned value is the event group value before any bits were automatically cleared in the case that x\+Clear\+On\+Exit parameter was set to pd\+TRUE.
\end{DoxyReturn}
Example usage\+: 
\begin{DoxyPre}
  \#define BIT\_0 ( 1 << 0 )
  ...
  const TickType\_t xTicksToWait = 100 / portTICK\_PERIOD\_MS;
    ...
    \{
        // \doxyref{xEventGroupWaitBits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returned because both bits were set.
    \}
    else if( ( uxBits \& BIT\_0 ) != 0 )
    \{
        // \doxyref{xEventGroupWaitBits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returned because just BIT\_0 was set.
    \}
    else if( ( uxBits \& BIT\_4 ) != 0 )
    \{
        // \doxyref{xEventGroupWaitBits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returned because just BIT\_4 was set.
    \}
    else
    \{
        // \doxyref{xEventGroupWaitBits()}{p.}{event__groups_8h_aab9d5b405bc57b7624dcabe9a9a503db} returned because xTicksToWait ticks passed
